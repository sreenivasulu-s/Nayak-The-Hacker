...
def _wordlist() -> str:
    ...
    for candidate in candidates:
        if candidate and Path(candidate).is_file():
            return candidate
    raise RuntimeError(
        "Gobuster wordlist not found. Set GOBUSTER_WORDLIST or install a standard wordlist."
    )
...
def _validate_scope(target: str, scope: str) -> None:
    target_host = _host(target).lower().rstrip(".")
    scope_host = _host(scope).lower().rstrip(".")
...
async def _run(command: list[str]) -> tuple[int, str, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"{command[0]} timed out after {TOOL_TIMEOUT_SECONDS} seconds"
        )
    ...
    for host in root.findall("host"):
        ...
        for port in host.findall("./ports/port"):
            ...
            findings.append(_finding(
                f"Open {port.attrib.get('portid')}/{port.attrib.get('protocol')} service",
                "info",
                "Nmap identified an open service on the authorized target.",
                evidence,
                "nmap",
            ))
    return findings
    ...
    for line in output.splitlines():
        ...
        if "Status:" in line or "Status:" in line:
            findings.append(_finding(
                "Web path discovered",
                "info",
                "Gobuster discovered a web path responding on the authorized target.",
                line,
                "gobuster",
            ))
    return findings
...
def _parse_nikto(output: str) -> list[dict]:
    ...
    for line in output.splitlines():
        ...
        if stripped.startswith("+") and any(token in stripped.lower() for token in ("osvdb", "x-frame-options", "cookie", "server", "allowed", "outdated", "interesting")):
            ...
            if "outdated" in lowered or "vulnerab" in lowered:
                severity = "medium"
            findings.append(_finding(
                "Nikto web-server observation",
                severity,
                "Nikto reported an observation for the authorized web target.",
                stripped,
                "nikto",
            ))
    return findings
...
def _parse_nuclei(output: str) -> list[dict]:
    ...
    for line in output.splitlines():
        ...
        evidence = item.get("matched-at") or item.get("host") or line
        findings.append(_finding(
            str(name),
            severity,
            str(info.get("description") or "Nuclei matched a template on the authorized target."),
            str(evidence),
            "nuclei",
        ))
    return findings
...
async def execute_tools(target: str, scope: str, tools: list[str]) -> tuple[list[dict], dict]:
    ...
    host = _host(target)
    web_target = _base_url(target)
    results: list[dict] = []
    evidence: dict = {}

    for tool in dict.fromkeys(selected):
        if tool == "nmap":
            code, out, err = await _run(["nmap", "-sV", "--version-light", "-oX", "-", host])
            results.extend(_parse_nmap(out))
            evidence[tool] = {"return_code": code, "stderr": err[-4000:], "raw_size": len(out)}
        elif tool == "gobuster":
            wordlist = _wordlist()
            code, out, err = await _run(["gobuster", "dir", "-u", web_target, "-w", wordlist, "--no-error", "-q"])
            results.extend(_parse_gobuster(out))
            evidence[tool] = {"return_code": code, "stderr": err[-4000:], "raw_size": len(out), "wordlist": wordlist}
        elif tool == "nikto":
            code, out, err = await _run(["nikto", "-h", web_target, "-Format", "txt"])
            results.extend(_parse_nikto(out))
            evidence[tool] = {"return_code": code, "stderr": err[-4000:], "raw_size": len(out)}
        elif tool == "nuclei":
            code, out, err = await _run(["nuclei", "-u", web_target, "-jsonl", "-silent"])
            results.extend(_parse_nuclei(out))
            evidence[tool] = {"return_code": code, "stderr": err[-4000:], "raw_size": len(out)}
    ...
    if not results:
        results.append(_finding(
            ...
            "info",
            "The selected tools completed without producing a parsed security finding.",
            json.dumps(evidence),
            "scanner-pipeline",
        ))
